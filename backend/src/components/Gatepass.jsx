...
const addGatepasss = (req, res) => {
  const { ref_number, purpose, destination, dot, departments, service_vehicle, names } = req.body;
  ...
  const q =
    "INSERT INTO gatepass (ref_number, purpose, destination, dot, departments, service_vehicle ,names) VALUES (?, ?, ?, ?, ?, ?, ?)";
  const values = [ref_number, purpose, destination, dot, departments, service_vehicle, names];
...
const deleteGatepass = (req, res) => {
  ...
  db.query(q, [gatepassId], (err, result) => {
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json({ message: "No gatepass found with this ID" });
    }

    return res.json({ message: "Gatepass has been deleted from your database." });
  });
};
